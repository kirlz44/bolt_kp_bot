...
module.exports = async (ctx) => {
  try {
    ...
    if (events.length === 0) {
      return ctx.editMessageText('Нет активных мероприятий для удаления', {
        reply_markup: {
          inline_keyboard: [
    ...
    const keyboard = events.map(event => ([{
      ...
      callback_data: `confirm_delete_event_${event.id}`
    }]));

    keyboard.push([{ text: '🔙 Назад', callback_data: 'manage_events' }]);

    await ctx.editMessageText('Выберите мероприятие для удаления:', {
      reply_markup: {
